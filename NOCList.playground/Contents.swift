...
let numberOfCompromised = compromisedAgents()

print("\(numberOfCompromised) agents have been compromised")
...
func findCleanAgents() -> Array<Any> {
    ...
    for agents in nocList {
        if agents.compromised == false {
            uncompromisedAgents.append(agents.realName)
...
print("\(findCleanAgents().count) clean agents out of \(nocList.count)")
